...
const filteredProducts = computed(() => {
    return products.value.filter(product => {
        ...
        const matchesCategory = !selectedCategory.value || product.id_categoria == selectedCategory.value;
...
const searchProducts = async () => {
    try {
        ...
        errorMessage.value = 'Error al buscar productos: ' + (error.response?.data?.message || error.message);
    } finally {
...
const handleCategoryChange = async () => {
    if (selectedCategory.value) {
        try {
            ...
            const productsResponse = await axios.get(`/admin/ruta/productos/${selectedCategory.value}`);
...
const addToCart = (product) => {
    ...
    const precioUnitario = product.descuento ? product.precio * (1 - (product.descuento / 100)) : product.precio;
...
const processSale = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            const invoiceResponse = await axios.get(`/admin/ventaLocal/factura/${response.data.venta_id}`);
...
<template>
    <div class="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 md:p-8">
        ...
        <div class="mb-6 space-y-4">
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white">Punto de Venta</h1>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="lg:w-2/3">
                ...
                <div v-else-if="filteredProducts.length === 0"
                    ...
                    <p class="text-xl text-gray-600 dark:text-gray-300">No se encontraron productos que coincidan con
                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <div v-for="product in filteredProducts" :key="product.id"
                        ...
                        <div class="space-y-2">
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-white">{{ product.nombre }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-300 line-clamp-2">{{ product.descripcion }}
                            </p>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-5">
                    ...
                    <div v-else class="space-y-3 max-h-[50vh] overflow-y-auto">
                        <div v-for="(item, index) in cart" :key="index"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <h4 class="font-semibold text-gray-800 dark:text-white text-sm">{{ item.nombre }}
                                    </h4>
                                    <p class="text-xs text-gray-600 dark:text-gray-300">Bs.{{ item.precioFinal }} c/u
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Método de
                                    Pago</label>
        ...
        <div v-if="showInvoice" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div id="invoice-print" class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-2xl w-full">
                <div class="flex justify-between items-start mb-6">
                    <h2 class="text-2xl font-bold dark:text-white">Factura #{{ invoiceData.id }}</h2>
                    <button @click="showInvoice = false"
                        class="text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-white">×</button>
                </div>
                ...
                <div class="space-y-4 mb-6">
                    <div class="dark:text-gray-200">
                        <p class="font-semibold">Fecha: {{ new Date().toLocaleDateString('es-BO') }}</p>
                        <p class="font-semibold">Hora: {{ new Date().toLocaleTimeString('es-BO') }}</p>
                        <p>Cliente: {{ invoiceData.cliente.nombre }}</p>
                        <p v-if="invoiceData.cliente.email">Email: {{ invoiceData.cliente.email }}</p>
                    </div>
                    ...
                    <div class="border-y border-gray-200 dark:border-gray-700 py-4">
                        <div v-for="(item, index) in invoiceData.items" :key="index"
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ item.almacen }} (x{{
                                    item.cantidad }})</p>
                            </div>
